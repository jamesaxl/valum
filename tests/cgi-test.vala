...
public void test_vsgi_cgi_request () {
	...
	var connection = new AutoCloseIOStream (new MemoryInputStream (), new MemoryOutputStream.resizable ());
...
public void test_vsgi_cgi_request_gateway_interface () {
	var request = new Request.from_cgi_environment (new AutoCloseIOStream (new MemoryInputStream (), new MemoryOutputStream.resizable ()), {"GATEWAY_INTERFACE=CGI/1.0"});
...
public void test_vsgi_cgi_request_content_type () {
	var request = new Request.from_cgi_environment (new AutoCloseIOStream (new MemoryInputStream (), new MemoryOutputStream.resizable ()), {"CONTENT_TYPE=text/html; charset=UTF-8"});
...
public void test_vsgi_cgi_request_content_length () {
	var request = new Request.from_cgi_environment (new AutoCloseIOStream (new MemoryInputStream (), new MemoryOutputStream.resizable ()), {"CONTENT_LENGTH=12"});
...
public void test_vsgi_cgi_request_content_length_malformed () {
	var request = new Request.from_cgi_environment (new AutoCloseIOStream (new MemoryInputStream (), new MemoryOutputStream.resizable ()), {"CONTENT_LENGTH=12a"});
